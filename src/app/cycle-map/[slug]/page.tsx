...
export default function CycleMapDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const getStepIcon = (label: string, className: string) => {
    const cleanLabel = label.trim();
    if (cleanLabel.includes('环境') || cleanLabel.includes('宏观')) {
      return <Globe className={className} size={32} />;
    }
    if (cleanLabel.includes('动能') || cleanLabel.includes('需求')) {
      return <Zap className={className} size={32} />;
    }
    if (cleanLabel.includes('库存') || cleanLabel.includes('水平') || cleanLabel.includes('水位')) {
      return <Box className={className} size={32} />;
    }
    if (cleanLabel.includes('价格') || cleanLabel.includes('定价') || cleanLabel.includes('反应')) {
      return <CircleDollarSign className={className} size={32} />;
    }
    if (cleanLabel.includes('供应') || cleanLabel.includes('开发') || cleanLabel.includes('矿端') || cleanLabel.includes('周期')) {
      return <Pickaxe className={className} size={32} />;
    }
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-white font-sans selection:bg-brand-blue/30 pb-32">
      {/* 注入全局动态关键帧 */}
      <style jsx global>{`
      ...
      <main className="max-w-7xl mx-auto px-8 py-12">
        ...
        {/* --- 工业头部 (Premium Header) --- */}
        <div id="overview" className="bg-[#141417] border border-white/10 p-12 rounded-3xl mb-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-12 relative z-10">
            ...
            <div className="flex items-end gap-16">
              ...
              <div className={`px-6 py-4 text-2xl font-black italic rounded-3xl shadow-[0_0_30px_rgba(0,0,0,0.5)] transform hover:scale-105 transition-transform ${data.change.startsWith('+') ? 'bg-emerald-500 text-slate-900 border-b-4 border-emerald-700' : 'bg-rose-500 text-white border-b-4 border-rose-700'}`}>
                {data.change}
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-4 gap-10">
            ...
            <div className="p-12 bg-white/[0.04] border border-white/10 rounded-3xl flex flex-col items-center justify-center text-center relative overflow-hidden group shadow-2xl transition-all hover:bg-white/[0.06]">
               {/* 动态雷达波扩散 */}
               ...
               <div className="px-6 py-2 bg-white/5 text-white/60 text-[10px] font-black rounded-3xl border border-white/10 uppercase italic tracking-[0.2em] z-10">Rating: Current Cycle Phase</div>
            </div>

            {/* 逻辑分解条形图 (High-Contrast Bar) */}
            <div className="md:col-span-3 p-12 bg-white/[0.04] border border-white/10 rounded-3xl shadow-2xl relative group">
        ...
        <div id="cycle" className="bg-[#141417] border border-white/10 p-16 rounded-3xl mb-10 shadow-2xl relative group">
                    ...
                    <React.Fragment key={idx}>
                        <div className={`flex-1 p-8 border rounded-3xl text-center transition-all duration-700 relative flex flex-col justify-center items-center group/card cursor-pointer hover:bg-white/[0.05] ${idx === data.cyclePosition ? 'border-brand-blue/50 bg-brand-blue/5 shadow-[0_0_60px_rgba(56,189,248,0.1)] scale-110 z-20 border-2' : 'border-white/5 bg-white/[0.01] opacity-40 hover:opacity-100 grayscale hover:grayscale-0'}`}>
                            ...
                            <div className="mb-6 flex justify-center items-center h-16 w-16">
                              {getStepIcon(pos.label, idx === data.cyclePosition ? "text-brand-blue transition-all duration-500 group-hover/card:scale-110 drop-shadow-[0_0_15px_rgba(56,189,248,0.3)]" : "text-white/40 group-hover/card:text-brand-blue transition-all duration-500 group-hover/card:scale-110")}
                            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-12">
                <div id="intelligence" className="p-12 border border-white/10 bg-[#141417] rounded-3xl shadow-2xl relative section-reveal">
            ...
            <div className="space-y-10">
                <div id="strategy" className="p-12 bg-brand-blue text-slate-900 border-l-[12px] border-slate-900/10 rounded-3xl shadow-[0_20px_50px_rgba(59,130,246,0.3)] relative overflow-hidden group/strat">
                ...
                <div id="signals" className="p-12 border border-white/10 bg-[#141417] rounded-3xl shadow-2xl relative overflow-hidden">
                    <div className="space-y-8">
                        <div className="p-8 bg-white/[0.04] border border-white/10 rounded-3xl hover:border-brand-blue/60 hover:bg-white/[0.06] transition-all cursor-cell group">
      ...
      <footer className="max-w-7xl mx-auto px-8 py-20 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] text-slate-500 font-mono tracking-widest uppercase">
        ...
        <Link href="/cycle-map" className="text-slate-500 hover:text-brand-blue transition-colors italic">
